...
> Статус: MVP 0.1 / этап 5. Этапы 1–4 реализованы; плагин ставится симлинком или zip в `Packages/RedVirtLogViewer`. Хосты, Engine-сущности и фильтры узла/уровня — вне MVP. Изменения решений необходимо отражать здесь вместе с изменением кода.
...
Настройки разделяются на общие и настройки сессии/источника. В MVP заданы:

- каталог кеша (`~/.cache/red-virt-log-viewer` или `XDG_CACHE_HOME`); **лимит объёма кеша не реализован и не измерялся**;
- `page_size`, `fragment_context_lines`;
- `sniff_bytes` / `sniff_lines`;
- кодировка и `journal_year` / `journal_utc_offset`;
- `max_line_bytes`, `direct_open_max_bytes`;
- исключения обхода каталогов в discovery.

Не объявлять подтверждённую поддержку многогигабайтных наборов по проверке фикстур и BosLPH.
...
7. Проверить UI, лимиты ресурсов, упаковку и пользовательскую документацию. **Сделано (этап 5, MVP 0.1).** Квота кеша не вводилась.
