...
> Статус: проект архитектуры MVP 0.1. Документ задаёт требования и решения для реализации. Он не подтверждает наличие работающего плагина. Изменения решений необходимо отражать здесь вместе с изменением кода.
...
Минимальная сборка Sublime, версия Python плагина и доступность `sqlite3`/`lzma` должны быть проверены до фиксации зависимостей. Отсутствующая возможность должна давать понятный статус, а не молчаливый пропуск файлов.
...
| `core/search.py` | Буквальный поиск, поиск объектов и проверка совпадений |
| `core/timeline.py` | Временные фильтры, нормализация и сортировка |
...
Сессия — контекст одного обращения, содержащий явно выбранные каталоги сборов. Сессия имеет собственный идентификатор, настройки и кеш. Окно Sublime выбирает активную сессию; результаты и фоновые задачи всегда содержат её идентификатор.
...
| journal | Варианты текстовых меток времени; отсутствие года или пояса отмечается отдельно |
...
Если пояс отсутствует, требуется настройка источника либо подтверждённое правило. Год в journal не восстанавливается из даты компьютера или архива молча. При необходимости пользователь задаёт год с учётом перехода между годами; неоднозначные записи остаются без нормализованного времени.
...
| `nodes` | Подтверждённые и неразрешённые узлы |
| `sources` | Связь сбора и узла, настройки времени и кодировки |
| `files` | Относительный путь, сжатие, размер, mtime, отпечаток, парсер и статус |
| `file_generations` | Поколения обработки файла и признак завершённости |
| `events` | Файл/поколение, время, уровень, строки, позиции и краткое представление |
...
| `identifiers` | Идентификаторы неизвестного назначения и их позиции |
...
Индекс обеспечивает поиск по подтверждённым упоминаниям. Если индекс неполон, выдача сообщает об этом и позволяет перейти к текстовому поиску. При вводе неизвестного UUID не требуется предварительное создание вымышленного объекта.
...
1. Уточнить окружение Sublime и проверить базовые зависимости.
2. Реализовать модели, discovery, readers и поиск с переходом к строке.
3. Добавить парсеры и обработку многострочных записей.
4. Добавить индекс с поколениями, состояниями и отменой.
5. Реализовать доказательства QEMU и поиск ВМ в VDSM.
6. Добавить объекты Engine, нормализацию времени и хронологию.
7. Проверить UI, лимиты ресурсов, упаковку и пользовательскую документацию.

Рабочий этап должен проверяться целиком от ввода до исходной строки. Наличие каркаса модулей не считается завершением MVP.
